...
public class UserDao {
	...
	public static User getUserById(int id) {
		...
		try {
			...
			Connection con = UserDao.getConnection();
			PreparedStatement pst = con.prepareStatement("Select * from user where u_id=?");
			pst.setInt(1, id);
			ResultSet result = pst.executeQuery();
			
			while(result.next()) {
				u.setId(result.getInt("u_id"));
				u.setFirstName(result.getString("first_name"));
				u.setLastName(result.getString("last_name"));
				u.setEmail(result.getNString("email"));
				u.setPassword(result.getString("password"));
				u.setMobile(result.getInt("mobile"));
				u.setBalance(result.getDouble("balance"));
				u.setDob(result.getString("dob"));
				u.setRole_id(result.getInt("role_id"));
			}
			con.close();
		} catch (Exception e) {
			e.printStackTrace();
